...
int main(int argc, char** argv) {
  ...
  *env << "LIVE555 Proxy Server\n"
  ...
  while (argc > 1) {
    ...
    case 'c': {
      // specify the desired camara to stream
      if (argc > 2 && argv[2][0] != '-') {
        // The next argument is the rtsp server port number:
        if (sscanf(argv[2], "%hu", &CameraNum) == 1
            && CameraNum > 0) {
          *env << "Successfully selected a camera nunber to stream\n";
          ++argv; --argc;
          break;
        }
      }
  ...
  authDB->addUserRecord("admin", "transmici2"); // replace these with real strings
  ...
  for (i = 1; i < argc; ++i) {
    ...
    char streamName[50];
    if (argc == 2) {
      sprintf(streamName, "%s", "Media/Live/Normal?camera=C_%hu&streamindex=1", CameraNum); // there's just one stream; give it this name
    } else {
      sprintf(streamName, "Media/Live/Normal?camera=C_%hu&streamindex=%d", CameraNum, i); // there's more than one stream; distinguish them by name
    }
